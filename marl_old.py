...
def train_agents(args):
    ...
    config = (
        ...
        .training(
            ...
            model={
                ...
                "fcnet_hiddens": [128, 128],  # Reduced from [256, 256]
            }
        )
        .multi_agent(
            policies={
            ...
            policy_mapping_fn=policy_mapping_fn  # Use the function defined above
        )
        .resources(
            num_gpus=1,
            # num_cpus=8,
        )
        .debugging(log_level="ERROR")  # Reduce logging
    )
    ...
    try:
        analysis = tune.run(
            ...
            storage_path='/home/zli/RPS/' + str(checkpoint_dir)+'/',
        ...
        metadata = {
            ...
            "best_checkpoint": last_trial.checkpoint.path if last_trial else None,
            "final_reward": last_trial.last_result.get("env_runners/episode_return_mean", None) if last_trial else None
        }
        ...
        print("pip install ray[rllib] torch gymnasium pygame")

class PygameVisualizer:
    """Pygame visualization for the RPS battle environment"""
...
def parse_arguments():
    ...
    parser.add_argument(
        "--num-workers", 
        type=int, 
        default=2,  # Reduced default for better compatibility
...
def main():
    ...
    # Initialize Ray
    if args.log:
        ray.init(num_cpus=13,ignore_reinit_error=True,logging_level=logging.DEBUG, log_to_driver=True)
    else:
        ray.init(num_cpus=13,ignore_reinit_error=True, log_to_driver=False)
